...
def apply_posture_mode():
    ...
    if posture_mode == "STAND":
        ...
        # Big left-right sway (roll)
        posture_roll_target = (
            SHOW_ROLL_AMP * math.sin(2 * math.pi * SHOW_FREQ_SWAY * t)
        )

        # Deep squat pulse (pitch)
        posture_pitch_target = (
            -SHOW_PITCH_AMP * abs(
                math.sin(2 * math.pi * SHOW_FREQ_SQUAT * t)
            )
        )

        # Knee pumping (visual exaggeration)
        for leg in FEET:
            motion_offsets[leg] = ramp(
                motion_offsets[leg],
                SHOW_KNEE_AMP * math.sin(2 * math.pi * SHOW_FREQ_SWAY * t),
                0.8
            )
...
def start_unified_gui():
    ...
    def start_show():
        global posture_mode, show_start_time, demo_active
        demo_active = False          # disable demo
        posture_mode = "SHOW"
