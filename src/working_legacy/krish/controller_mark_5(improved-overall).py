...
# -------------------------------------------------
# BODY SHIFT CONFIG  (lean into movement direction)
# -------------------------------------------------
# All values in metres.  Body shifts toward the direction
# of travel; in foot-space that means all feet move opposite.
# These are the offsets at full-scale input (±1.0).
...
def wrap_phase(p: float) -> float:
    """Wrap phase into [0, 1).  Guards the float edge at exactly 1.0."""
    w = p % 1.0
    # % can return 1.0 for values indistinguishable from an integer
...
def _lateral_trajectory(phase, step_length, step_height, duty):
    # Clamp to [0, 1) — defends against float edge from wrap_phase
...
def compute_feet_directional(phase, direction, stance_z=STANCE_Z):
    """Route D-pad / keyboard direction strings through the unified analog engine."""
    if direction == "forward":
        return compute_feet_analog(phase,  1.0, 0.0, 0.0, 1.0, stance_z)
    elif direction == "backward":
        return compute_feet_analog(phase, -1.0, 0.0, 0.0, 1.0, stance_z)
    elif direction == "left":
        return compute_feet_analog(phase, 0.0, -1.0, 0.0, 1.0, stance_z)
    elif direction == "right":
        return compute_feet_analog(phase, 0.0,  1.0, 0.0, 1.0, stance_z)
    elif direction == "turn_left":
        return compute_feet_analog(phase, 0.0, 0.0, -1.0, 1.0, stance_z)
...
def compute_feet_analog(phase: float,
    ...
    # --- Body shift: lean into movement direction ---
    # In body-frame, shifting the body forward means all feet
    # move backward relative to the body → subtract from X.
    shift_x = -fwd    * BODY_SHIFT_FWD
    shift_y = -strafe * BODY_SHIFT_LAT
    # Turn: lean sideways into the arc (turn right → shift right → feet go -Y)
    shift_y += -turn  * BODY_SHIFT_TURN
