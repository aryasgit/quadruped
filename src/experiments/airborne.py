...
AIRBORNE_RISE = 0.10         # how fast airborne confidence rises
AIRBORNE_FALL = 0.18         # how fast it decays
...
while True:
    try:
        ...
        # Angular-only brace trigger (ignore linear translation)
        roll_rate  = gx / 131.0
        pitch_rate = gy / 131.0
        ...
        for leg in FEET:
            ...
            # 🔒 FREEZE contact estimation during brace
            if brace_active:
                continue
        ...
        # AUTHORITATIVE airborne inference (non-gimmicky)
        # AUTHORITATIVE airborne inference (brace-safe)
        if brace_active:
            # Brace is an external disturbance — never infer airborne here
            airborne_conf = max(0.0, airborne_conf - AIRBORNE_FALL)

        else:
            if avg_contact < 0.35:
                airborne_conf = min(1.0, airborne_conf + AIRBORNE_RISE)

            elif free_legs >= 3 and roll_effort < 0.06:
                airborne_conf = min(1.0, airborne_conf + AIRBORNE_RISE)
        ...
        assert not (brace_active and airborne_conf > 0.2), \
            "Brace must never degrade ground confidence"
