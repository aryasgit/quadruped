...
def start_monitor_gui():
    ...
    # Foot contact labels
    contact_frame = tk.LabelFrame(root, text="Foot Contact (0–1)")
    contact_frame.pack(padx=10, pady=5, fill="x")

    contact_vars = {}
    for leg in FEET:
        var = tk.StringVar()
        contact_vars[leg] = var
        tk.Label(
            contact_frame,
            textvariable=var,
            font=("Arial", 10),
    ...
    def update():
        ...
        for leg, var in contact_vars.items():
            var.set(f"{leg}: {monitor_state['foot_contact'].get(leg, 0.0):.2f}")
...
while True:
    try:
        ...
        for leg, ch in FEET.items():
            ...
            '''# DEBUG — per-leg servo delta (VALID PLACE)
            servo_delta = abs(angle - last_servo_angles[ch])
            last_servo_angles[ch] = angle

            if leg == "FRF":
                print(
                    f"[FRF DEBUG] FSM={fsm_state} "
                    f"servo_delta={servo_delta:.3f} "
                    f"pitch_offset={pitch_offsets[leg]:.3f} "
                    f"motion_offset={motion_offsets[leg]:.3f}"
                )'''
        ...
        # ===== CONTACT DEBUG (TIME-BASED, STABLE) =====
        now = time.time()
        if (now - last_contact_print) > CONTACT_PRINT_DT:
            if fsm_state in ("LIFTING", "RECENTERING", "IDLE"):
                print(
                    f"[CONTACT] FSM={fsm_state} | "
                    f"FRF={foot_contact['FRF']:.2f} "
                    f"FLF={foot_contact['FLF']:.2f} "
                    f"RRF={foot_contact['RRF']:.2f} "
                    f"RLF={foot_contact['RLF']:.2f}"
                )
            last_contact_print = now
        time.sleep(DT)
