...
from math import *
from ik.util import RotMatrix3D, point_to_rad
...
class kinematics():
    ...
    def leg_IK_calc(self, xyz, is_right=False): 
        ...
        if len_B >= (self.link_2 + self.link_3):
            # Target beyond reach — soft-clamp to the workspace. (Was printing every
            # control tick and flooding stdout; the clamp alone is the fix.)
        ...
        # BUG FIX: clamp acos arguments to [-1, 1]. For near/short targets these
        # ratios can drift just past +/-1 due to float error (or a target inside
        # |l2-l3|), raising "math domain error" and crashing the gait loop.
        _c2 = (self.link_2**2 + len_B**2 - self.link_3**2) / (2 * self.link_2 * len_B)
        _c3 = (self.link_2**2 + self.link_3**2 - len_B**2) / (2 * self.link_2 * self.link_3)
        b_2 = acos(max(-1.0, min(1.0, _c2)))
        b_3 = acos(max(-1.0, min(1.0, _c3)))
    ...
    def angle_corrector(self, angles=[0,0,0], is_right=True):
        ...
        if is_right:
            theta_1 = a[0] - pi
            theta_2 = a[1] + 45 * pi / 180
        else:
            theta_1 = a[0] - 2 * pi if a[0] > pi else a[0]
            theta_2 = -a[1] - 45 * pi / 180
            # Note: left/right thigh signs are opposite here by design.
            # joint_conventions.py uses +1 for all thighs to preserve this
            # asymmetry, which the servo mount inversion then makes symmetric
            # end-to-end. Do not add a -1 sign in joint_conventions for right thighs.

        theta_3 = -a[2] + 45 * pi / 180
